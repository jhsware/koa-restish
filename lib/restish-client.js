...
class Client {
  ...
  _findObjects (inp) {
    ...
    if (Array.isArray(inp)) {
      ...
      outp = outp.concat(Object.keys(inp)
        .filter((key) => typeof inp[key] === 'object')
        .map((key) => this._findObjects(inp[key])))
    }
    return outp
  }
  ...
  query (opts) {
    ...
    const outp = inp.map(({ URI, query, shape, cache = true}) => {
      let queryCacheKey
      if (query) {
        assert(typeof query === 'object')
        queryCacheKey = this._toCacheKey(query)
      }
      ...
      if (cache) {
        if (this.queryCache.hasOwnProperty(cacheKey)) {
          ...
          const data = this._fetchFromCache(cacheKey)
          return {URI, method: 'query', query, shape, result: data}
        }
      }

      return {URI, method: 'query', query, shape, cacheKey}
    })
